...
from subckt import Subckt, Device, Net
from circuits2graph import run_cir2g
import os
import warnings
import re

def findInterNetName(netCapDict: dict(), subList:list(), subIdx: int, 
                      prefix : str, y:list() = None): 
    ...
    for device in subckt.deviceList:
        ...
        assert cSubIdx >= 0
        findInterNetName(netCapDict, subList, cSubIdx, prefix+device.name)#, y)
...
if __name__ == '__main__':
    moduleName = '8T_digitized_timing_top_fast' #'pe_macro2' #'ssram' #'array_128_32_8t' # pe_macro2 # ultra_8T_macro #
    subcktList = read_cdl('/data1/shenshan/SPF_examples_cdlspf/CDL_files/ultra8T_cdl_lvs/'+moduleName+'.cdl')
    subIdx = -1
    ...
    net_cap_dict = read_spf('/data1/shenshan/SPF_examples_cdlspf/SPF_files/ultra_8T/'+moduleName+'.spf')
    assert (len(net_cap_dict))
    
    print('Calling findInterNetName...')
    findInterNetName(net_cap_dict, subcktList, subIdx, '')
    # for i, net in enumerate(subcktList[subIdx].netList):
    #     print(net)

    print('Converting to a graph...')
    run_cir2g(subcktList, subIdx)

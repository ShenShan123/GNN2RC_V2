...
def plot_errors(x, y, pltname):
    ...
    fig = plt.scatter(x.tolist(), y.tolist(), marker=".")
    plt.xlabel('relative errors')
    plt.ylabel('cap (fF)')
    plt.grid()
    # plt.xlim([0, 2])
    max_err, _ = torch.max(x, dim=0)
    plt.title('mean error:{:.4f}%, max error:{:.4f}%, 95% quantile:{:.4f}%' \
              .format(torch.mean(x).item()*100, max_err.item()*100, \
                      torch.quantile(x, 0.95).item()*100))
...
def validation(logits, targets, mask, pltname="err_in_val"):
    with torch.no_grad():
        ...
        metrics = {"mean_err:" mean_err.item(), "max_err": max_err}
        return metrics
...
if __name__ == '__main__':
    ...
    for i in range(1):
        ...
        # model_list.append(GAT(proj_feat, gnn_feat, gnn_feat, heads=[4,1], feat_drop=0.3, attn_drop=0.6).to(device))
        # model_list.append(GraphSAGE(proj_feat, gnn_feat, gnn_feat, 2, activation=nn.ReLU(), dropout=0.4, aggregator_type="mean"))
